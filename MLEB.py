...
if df_kunjungan is not None:
    ...
    if menu == "🏠 Overview":
        ...
        # --- REVISI: REVENUE PER PROGRAM (BAR CHART + CATATAN KEPANJANGAN) ---
        ...
        # Diurutkan agar yang menghasilkan revenue tertinggi berada di paling atas
        df_prog_pie = df_prog_pie.sort_values('Total Revenue', ascending=True)
        
        # Membuat Bar Chart Horizontal yang Menarik
        fig_prog_bar = px.bar(
            df_prog_pie,
            x='Total Revenue',
            y='Program',
            ...
            text='Total Revenue',
            color='Total Revenue',
            color_continuous_scale='Blues' # Menggunakan gradasi warna biru yang elegan
        )
        
        fig_prog_bar.update_traces(texttemplate='Rp %{text:,.0f}', textposition='outside')
        fig_prog_bar.update_layout(
            showlegend=False, 
            coloraxis_showscale=False, 
            yaxis_title=None,
            xaxis_title="Total Pendapatan (Rupiah)",
            margin=dict(l=20, r=40, t=20, b=20)
        )
        st.plotly_chart(fig_prog_bar, use_container_width=True)

        # Tambahan: Catatan Kepanjangan Nama Program Resmi MLEB
        with st.expander("📖 Lihat Kepanjangan / Glosarium Nama Program"):
            st.markdown("""
            * **MEE** : Museum Explorer & Eduventure
            * **5D** : 5D Theatre
            * **BUNDLING** : Bundling (Museum & 5D Theatre)
            * **MA** : Museum Adventure
            * **FTH** : Family Treasure Hunt
            * **CMP** : Corporate Meeting Package
            * **GG** : Gather & Grow
            * **PSR** : Power Splash Run
            * **MMN** : Museum Movie Night
            * **CDM** : Couple Date Museum
            * **EEMCE** : Energy Explorer - Museum Camping Experience
            * **CTMPC** : Capture The Museum - Photography Challenge
            * **EFE** : Energy Future Expo
            * **ETS** : Energy Transition Summit
            * **ER** : Escape Room : Misi Penyalaan Kota
            """)
